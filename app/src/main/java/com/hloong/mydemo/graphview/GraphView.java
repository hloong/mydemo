...
package com.hloong.mydemo.graphview;
...
import com.hloong.mydemo.graphview.GraphViewStyle.GridStyle;
 ...
 * {@link #drawSeries(Canvas, GraphViewDataInterface[], float, float, float, double, double, double, double, float, com.andy.graph_view.GraphViewSeries.GraphViewSeriesStyle)}
 * to display a custom graph. Use {@link com.andy.graph_view.LineGraphView} for
...
abstract public class GraphView extends LinearLayout {
	static final public class GraphViewConfig {
		...
		// 圆角矩形 半径 单位dp
		static final int RECT_RADIUS = 4;
		// 圆角矩形背景
		static final int RECTF_RADIUS = 12;
	}
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			double minY = getMinY() - getMinY() * 0.2;
			double maxY = getMaxY() + getMaxY() * 0.2;
			// double maxY = 9d;
			// double minY = 5d;
			...
			// ///////////////////////////////////////////////////////////////////////
			// 画水平线
			int hors = horlabels.length; // y轴标签向内偏移修改
			// y轴标签向内偏移修改 显示标签
			...
			if (graphViewStyle.getGridStyle() != GridStyle.HORIZONTAL) {
				...
				for (int i = 0; i < verlabels.length; i++) {
					...
					if (i == 1 || i == 3 || i == 5) {
						canvas.drawLine(horstart + offX, y, getWidth(), y,paint);// y轴标签向内偏移修改
					} else {
						canvas.drawLine(horstart + offXx, y, getWidth(), y,paint);
					}
				}
			}
...
//				MyLog.i("iiss", "=========i" + count++ + i);
				if (i == 1 || i == 3 || i == 5) {
					canvas.drawText(String.valueOf(decimalFomat.format(Double.parseDouble(verlabels[i]))), labelsOffset, y+ DensityUtil.dip2px(context, 5), paint);
				} else {
//					MyLog.i("iiss", "=========i" + count++ + i);
				}
	...
	synchronized private String[] generateVerlabels(float graphheight) {
		...
		double min = getMinY() - getMinY() * 0.2;
		double max = getMaxY() + getMaxY() * 0.2;
		// double max = 9d;
		// double min = 5d;
		if (max == min) {
			// if min/max is the same, fake it so that we can render a line
